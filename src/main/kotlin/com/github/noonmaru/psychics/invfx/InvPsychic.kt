...
object InvPsychic {
    ...
    fun create(psychicConcept: PsychicConcept, stats: (EsperStatistic) -> Double): InvScene {
        return InvFX.scene(1, "${ChatColor.BOLD}Psychic") {
            panel(0, 0, 9, 1) {
                onInit = {
                    setItem(0, 0, psychicConcept.renderTooltip().toItemStack(ItemStack(Material.ENCHANTED_BOOK)))
                }
                listView(2, 0, 5, 1, true, psychicConcept.abilityConcepts) {
                    transform = { renderTooltip(stats).toItemStack(wand ?: ItemStack(Material.BOOK)) }
